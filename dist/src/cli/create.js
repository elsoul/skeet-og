...
const init = async (appName) => {
    ...
    await (0, execSyncCmd_1.execSyncCmd)(gitCloneCmd);
    const yarnCmd = ['yarn'];
    await (0, execSyncCmd_1.execSyncCmd)(yarnCmd, appDir);
    const rmDefaultGit = ['rm', '-rf', '.git'];
    await (0, execSyncCmd_1.execSyncCmd)(rmDefaultGit, appDir);
    await (0, exports.generateInitFiles)(appName);
    const createNetworkCmd = ['docker', 'network', 'create', 'skeet-network'];
    await (0, execSyncCmd_1.execSyncCmd)(createNetworkCmd);
...
const generateInitFiles = async (appName) => {
    ...
    await (0, execSyncCmd_1.execSyncCmd)(rmDefaultEnv, apiDir);
...
const createGitHubRepo = async (repoName, openSource = false) => {
    ...
    await (0, execSyncCmd_1.execSyncCmd)(cmdLine);
};
...
const runPsql = async () => {
    ...
    await (0, execSyncCmd_1.execSyncCmd)(runPsqlCmd);
...
const initDbMigrate = async (apiDir) => {
    ...
    await (0, execSyncCmd_1.execSyncCmd)(prismaMigrateCmd, apiDir);
};
